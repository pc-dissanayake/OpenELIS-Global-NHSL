...
This document provides instructions for setting up and configuring the
OpenELIS-Global-2 application, including Docker-based development environment,
UI rebranding, FHIR integration for patient data retrieval, and current
...
- Updated `BANNER_TEXT` in
  `/opt/workspace/OpenELIS-Global-2/volume/properties/SystemConfiguration.properties`
...
A new REST controller `FhirProxyController.java` has been added to proxy FHIR
requests:

- Location:
  `/opt/workspace/OpenELIS-Global-2/src/main/java/org/openelisglobal/common/rest/FhirProxyController.java`
...
- Testing shows endpoint returns data, but currently serving HTML instead of
  JSON (potential nginx routing issue)
...
Expected: JSON response with FHIR Bundle containing patient data Current:
Returns HTML (React app fallback) - indicates routing issue
